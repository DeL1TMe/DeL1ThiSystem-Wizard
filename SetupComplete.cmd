...
echo ==========================================================>>"%LOG%"
echo [%date% %time%] [SETUP] SetupComplete started>>"%LOG%"
if exist "%EXE%" (
  echo [%date% %time%] [SETUP] EXE found: "%EXE%">>"%LOG%"
  powershell.exe -NoProfile -Command ^
    "try { (Get-Item -LiteralPath '%EXE%').VersionInfo.FileVersion } catch { '' }" >>"%LOG%" 2>&1
) else (
  echo [%date% %time%] [SETUP] WARN: EXE missing: "%EXE%">>"%LOG%"
)
...
if /i not "%CUR_NAME%"=="%TARGET_NAME%" (
  echo [%date% %time%] [SETUP] Renaming computer from "%CUR_NAME%" to "%TARGET_NAME%">>"%LOG%"
  powershell.exe -NoProfile -Command ^
    "try { Rename-Computer -NewName '%TARGET_NAME%' -Force -ErrorAction Stop } catch { Write-Output $_.Exception.Message; exit 1 }" >>"%LOG%" 2>&1
) else (
  echo [%date% %time%] [SETUP] Computer name already "%TARGET_NAME%">>"%LOG%"
)

if exist "%WALL_DARK%" (
  echo [%date% %time%] [SETUP] Set default wallpaper: "%WALL_DARK%">>"%LOG%"
  ...
  echo [%date% %time%] [SETUP] WARN: Default wallpaper missing: "%WALL_DARK%">>"%LOG%"
)

if exist "%LOCK_DARK%" (
  echo [%date% %time%] [SETUP] Set lockscreen: "%LOCK_DARK%">>"%LOG%"
  ...
  echo [%date% %time%] [SETUP] WARN: Lockscreen missing: "%LOCK_DARK%">>"%LOG%"
)

echo [%date% %time%] [SETUP] Set user GeoID (Germany, 94)>>"%LOG%"
...
echo [%date% %time%] [SETUP] Disable Edge first run/autostart>>"%LOG%"
reg add "HKLM\SOFTWARE\Policies\Microsoft\Edge" /v HideFirstRunExperience /t REG_DWORD /d 1 /f >nul 2>&1
reg add "HKLM\SOFTWARE\Policies\Microsoft\Edge" /v AutoImportAtFirstRun /t REG_DWORD /d 4 /f >nul 2>&1
...
reg add "HKLM\SOFTWARE\Policies\Microsoft\Edge" /v RemoveDesktopShortcutDefault /t REG_DWORD /d 1 /f >nul 2>&1

if exist "%BASE%\WizardTask.xml" (
  schtasks /Query /TN "%BRAND%\Wizard" >nul 2>&1
  if errorlevel 1 (
    echo [%date% %time%] [SETUP] Creating task "%BRAND%\Wizard">>"%LOG%"
    schtasks /Create /F /TN "%BRAND%\Wizard" /XML "%BASE%\WizardTask.xml">>"%LOG%" 2>&1
  ) else (
    echo [%date% %time%] [SETUP] Updating task "%BRAND%\Wizard">>"%LOG%"
    schtasks /Delete /F /TN "%BRAND%\Wizard" >nul 2>&1
    schtasks /Create /F /TN "%BRAND%\Wizard" /XML "%BASE%\WizardTask.xml">>"%LOG%" 2>&1
  )
) else (
  echo [%date% %time%] [SETUP] ERROR: Missing "%BASE%\WizardTask.xml">>"%LOG%"
)

reg delete "HKLM\SOFTWARE\Microsoft\Active Setup\Installed Components\DeL1ThiSystemWizard" /f >nul 2>&1

echo [%date% %time%] [SETUP] SetupComplete finished>>"%LOG%"
exit /b 0
